...
> Не коммитьте `.env` и `dev.db`. Секреты только в Vercel / Neon.
...
Опционально: `SMTP_*`, `TURNSTILE_*` — см. `.env.example`.

6. **Deploy**

При сборке выполняется `prisma db push` — таблицы создаются в Neon автоматически.
...
Логин в админку: `admin@academy.ua` / `admin123` — **смените пароль после первого входа**.
...
На Vercel файлы в `public/uploads/` **не сохраняются** между деплоями. Статика в `public/images/` и `public/documents/` в репозитории работает. Новые загрузки через админку — позже подключить Vercel Blob.
...
Без ключей в dev капча отключается.
